...
──────────────                ──────────────                ─────────────
trace.c hooks ──► shm ring ──► trace_stream ──zstd/TCP──► callscope serve
...
uv tool install 'callscope[stream]'
callscope serve --port 9001 --out traces/
...
./callscope/trace_stream /tracekit0 <server-ip> 9001 &
...
`callscope serve` prints one line per connection with the event count.
...
- **Same analysis path.** The server writes standard
  `trace.stream.*.bin` files; `callscope analyze` and the web UI consume
