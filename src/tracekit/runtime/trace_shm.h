#ifndef TRACEKIT_TRACE_SHM_H
#define TRACEKIT_TRACE_SHM_H

/*
 * Shared-memory ring layout and stream wire protocol for tracekit
 * remote tracing.
 *
 ...
 * streams them over raw TCP to a tracekit server (`tracekit serve`).
 *
...
/*
 * Open or create the ring `name` (POSIX shm name, e.g. "/tracekit0") with
...
static trace_shm_header_t *trace_shm_attach(const char *name,
    ...
    if (created) {
        ...
        fprintf(stderr, "tracekit: shm %s has incompatible layout\n", name);
        return NULL;
    }
    return h;
}
